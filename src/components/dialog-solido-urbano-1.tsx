...
import type { ReporteI, ResiduoSolido1PdfI } from "@/interfaces/interfaces";
import { generarReporteRSU } from "../api/service";


interface PropI {
  open: boolean,
  setOpen: (prev: boolean) => void;
  data : ResiduoSolido1PdfI
}

export function DialogSolidoUrbano1({open, setOpen, data } : PropI) {
  ...
  const getReporte = async() => {
    const result = await generarReporteRSU(data);
