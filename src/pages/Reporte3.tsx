...
//listado
import ListadoManejoEspecialRME from "@/app/estadisticoRME/page";

export function Reporte3() {
  ...
  const cargarEstadistica = async () => {
    ...
    const result = await crearEstadisticoRME(dataToSend);
  ...
  const exportarGrafica = () => {
    ...
    // título
    pdf.setFontSize(16)
    pdf.text("Reporte Residuo Especial (RME)", margin, 15)
    ...
    // guardar pdf
    pdf.save(`ResiduosRME_${timestamp}.pdf`)

    //excel
    const worksheet = XLSX.utils.json_to_sheet(data)
    const workbook = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(workbook, worksheet, "Datos")
    XLSX.writeFile(workbook, `ResiduosRME_${timestamp}.xlsx`)
  }
  ...
  const cargarListado = async () => {
    const result = await creaReporteListadoRME(filtros)
    if (result) {
    ...
    <div className="p-5">
      ...
      <Card className="mt-2 h-full w-full shadow-md">
        <CardContent>
          ...
          <ListadoManejoEspecialRME data={data} />
        </CardContent>
      </Card>
    </div>
  )
