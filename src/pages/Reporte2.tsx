...
//listado
import ListadoManejoEspecialRSU from "@/app/estadisticosRSU/page"

export function Reporte2() {
  //catalogos
  ...
  const cargarEstadistica = async () => {
    ...
    const result = await crearEstadisticoRSU(dataToSend)
  ...
  const exportarGrafica = () => {
    ...
    // título
    pdf.setFontSize(16)
    pdf.text("Reporte Sólido Especial (RSU)", margin, 15)
    ...
    // guardar pdf
    pdf.save(`ResiduosRSU_${timestamp}.pdf`)

    //excel
    const worksheet = XLSX.utils.json_to_sheet(data)
    const workbook = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(workbook, worksheet, "Datos")
    XLSX.writeFile(workbook, `ResiduosRSU_${timestamp}.xlsx`)
  }
    ...
    <div className="p-5">
      ...
      <Card className="mt-2 h-full w-full shadow-md">
        <CardContent>
          ...
          <ListadoManejoEspecialRSU data={data} />
        </CardContent>
      </Card>
    </div>
  )
